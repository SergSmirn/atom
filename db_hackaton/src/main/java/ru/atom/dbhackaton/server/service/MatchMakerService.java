...
import ru.atom.dbhackaton.server.model.Match;
...
import ru.atom.dbhackaton.server.dao.UserResultDao;
    ...
    public void finish(String jsonString) throws RuntimeException {
        ...
        try (Session session = Database.session()) {
            ...
            for (User user : results.keySet()) {
                UserResult personalResult = new UserResult(match, user, results.get(user));
                UserResultDao.getInstance().insert(session, personalResult);
            }
            txn.commit();
